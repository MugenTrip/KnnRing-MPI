	...
	for(int i=0;i<world_size;i++)
	{
    	if(i==0)
    	{
    		...
    		if(world_rank==0)
    		{
    			...
    			MPI_Send(static_data ,n*d , MPI_DOUBLE , next , 0 , MPI_COMM_WORLD );
           		MPI_Recv(dynamic_data , n*d, MPI_DOUBLE,prev,0,MPI_COMM_WORLD , &status );
    		}
    		else
    		{
	            MPI_Recv(dynamic_data , n*d, MPI_DOUBLE,prev,0,MPI_COMM_WORLD , &status );
        ...
        else if (i==world_size-1)
        {
            ...
            if(world_rank==0)
    		{
    			MPI_Send(temp_data ,n*d , MPI_DOUBLE , next , 0 , MPI_COMM_WORLD );
           		MPI_Recv(dynamic_data , n*d, MPI_DOUBLE,prev,0,MPI_COMM_WORLD , &status );
 			}
    		else
    		{
